...
export function createTradeRequest(
  ...
  if (filters.altQuality && !filters.altQuality.disabled) {
    switch (filters.altQuality.value) {
      case 'Superior':
        prop.set(
          query.filters,
          'misc_filters.filters.gem_alternate_quality.option',
          '0'
        )
        break
      case 'Anomalous':
  ...
  if (filters.areaLevel) {
    prop.set(
      query.filters,
      'heist_filters.filters.area_level.min',
  ...
  if (filters.heistJob) {
    switch (filters.heistJob.name) {
      case 'Agility':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_agility.min',
          filters.heistJob.level
        )
        break
      case 'Brute Force':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_brute_force.min',
          filters.heistJob.level
        )
        break
      case 'Counter-Thaumaturgy':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_counter_thaumaturgy.min',
          filters.heistJob.level
        )
        break
      case 'Deception':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_deception.min',
          filters.heistJob.level
        )
        break
      case 'Demolition':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_demolition.min',
          filters.heistJob.level
        )
        break
      case 'Engineering':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_engineering.min',
          filters.heistJob.level
        )
        break
      case 'Lockpicking':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_lockpicking.min',
          filters.heistJob.level
        )
        break
      case 'Perception':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_perception.min',
          filters.heistJob.level
        )
        break
      case 'Trap Disarmament':
        prop.set(
          query.filters,
          'heist_filters.filters.heist_trap_disarmament.min',
          filters.heistJob.level
        )
        break
    }
    ...
    for (const influence of filters.influences) {
      ...
      switch (influence.value) {
        case ItemInfluence.Shaper:
          prop.set(
            query.filters,
            'misc_filters.filters.shaper_item.option',
            String(true)
          )
          break
        case ItemInfluence.Elder:
          prop.set(
            query.filters,
            'misc_filters.filters.elder_item.option',
            String(true)
          )
          break
        case ItemInfluence.Crusader:
          prop.set(
            query.filters,
            'misc_filters.filters.crusader_item.option',
            String(true)
          )
          break
        case ItemInfluence.Hunter:
          prop.set(
            query.filters,
            'misc_filters.filters.hunter_item.option',
            String(true)
          )
          break
        case ItemInfluence.Redeemer:
          prop.set(
            query.filters,
            'misc_filters.filters.redeemer_item.option',
            String(true)
          )
          break
        case ItemInfluence.Warlord:
          prop.set(
            query.filters,
            'misc_filters.filters.warlord_item.option',
            String(true)
          )
          break
      }
  ...
  for (const stat of stats) {
    if (stat.tradeId[0] === 'map.no_elder_guardian') {
      query.stats.push({
        ...
        filters: [
          tradeIdToQuery(
            STAT_BY_REF.get('Map is occupied by #')!.types[0].tradeId[0],
            stat
          )
        ]
      })
    ...
    switch (stat.tradeId[0] as INTERNAL_TRADE_ID) {
      case 'armour.armour':
        prop.set(
          ...
          typeof stat.min === 'number' ? stat.min : undefined
        )
        prop.set(
          query.filters,
          'armour_filters.filters.ar.max',
          typeof stat.max === 'number' ? stat.max : undefined
        )
        break
...
export async function requestTradeResultList(
  ...
  const response = await fetch(
    `${
      MainProcess.CORS
    }https://${getTradeEndpoint()}/api/trade/search/${leagueId}`,
    {
      method: 'POST',
      headers: {
  ...
  RATE_LIMIT_RULES.SEARCH = adjustRateLimits(
    RATE_LIMIT_RULES.SEARCH,
    response.headers
  )
...
export async function requestResults(
  ...
  const response = await fetch(
    `https://${getTradeEndpoint()}/api/trade/fetch/${resultIds.join(
      ','
    )}?query=${queryId}`
  )
  RATE_LIMIT_RULES.FETCH = adjustRateLimits(
    RATE_LIMIT_RULES.FETCH,
    response.headers
  )
  const data: {
    result: FetchResult[]
